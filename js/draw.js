...
function createRateChart() {
  ...
  svg.append("text")
    ...
    .attr("dy", "1em")
    .style("text-anchor", "middle")
    .text("The number of closed restaurants");
  ...
  const dataFile = "data/data_clean.csv"
  ...
  d3.csv(dataFile, function(error, allData) {
    allData = allData.filter(function(d) {
      return d["city"] == "Phoenix" && d["is_closed"] == "True";
    });
    allData.forEach(function(d) {
      d.Name = d.business_id
      d.Value = +d.stars;
    });
    ...
    //In total, there are 270 closed restaurants in Phoenix.
    // console.log(data);
...
function createReviewChart() {
  ...
  function barPrimary(viz4data1) {
    ...
    var xScale = d3.scaleBand()
      .domain(viz4data1.map(function(d) {
    ...
    var yScale = d3.scaleLinear()
      .range([height, 0])
      .domain(d3.extent(viz4data1, function(d) {
        return +d.zreview_count_all;
      })).nice()
    ...
    svg.selectAll('.bar')
      ...
      .attr("class", function(d) {
        return "bar bar--" + (+d.zreview_count_all < 0 ? "negative" : "positive");
      })
      .attr('fill', 'navy-blue')
      .attr('x', function(d) {
        return xScale(d.business_id);
      })
      .attr('width', xScale.bandwidth())
      .attr("height", function(d) {
        return Math.abs(yScale(0) - yScale(+d.zreview_count_all));
      })
      .attr("y", function(d) {
        return (height / 2 - yScale(Math.min(0, +d.zreview_count_all)) + yScale(+d.zreview_count_all))
      })
    ...
    svg.selectAll('text.label')
      ...
      .append('text')
      .attr("class", function(d) {
        return "label label--" + (+d.zreview_count_all < 0 ? "negative" : "positive");
      })
      .text(function(d) {
        return d.business_id;
      })
